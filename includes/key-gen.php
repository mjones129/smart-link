...
function my_plugin_generate_encryption_key() {
    return bin2hex(openssl_random_pseudo_bytes(32));
}

// Function to write the encryption key to wp-config.php
function my_plugin_write_encryption_key_to_config() {
    $key = my_plugin_generate_encryption_key();
    ...
    if (is_writable($config_path)) {
        ...
        if (strpos($config_file, 'define(\'MY_PLUGIN_ENCRYPTION_KEY\'') === false) {
            $key_define = "\ndefine('MY_PLUGIN_ENCRYPTION_KEY', '$key');\n";
...
register_activation_hook(__FILE__, 'my_plugin_write_encryption_key_to_config');
